...
def _read_block_with_header(inst):
    """
    Read a LeCroy binary block starting with #<N><len><data>
    Returns raw bytes of the data payload (no header).
    """
    # Read first 2 bytes to get '#' and N
    first = inst.read_raw(2)
    if not first or first[0:1] != b'#':
        raise RuntimeError(f'Unexpected header start: {first}')
    n_digits = int(first[1:2])
    # Read the length field (n_digits bytes)
    len_bytes = inst.read_raw(n_digits)
    data_len = int(len_bytes.decode('ascii'))
    # Read data payload
    data = inst.read_raw(data_len)
    return data
