...
files = glob.glob('output/LF_PreIrrad_Sr90_glich_80uV_with_foil_HW/*.root')
...
fig2, ax2 = plt.subplots(figsize=(10, 8)) # waveform
fig3, ax3 = plt.subplots(figsize=(10, 8)) # charge

# pedestal
ax2.set_xlabel('Bias Voltage (V)')
ax2.set_ylabel('Count (V)')
...
for j in range(len(v_sorted)): # file loop
    ...
    for i in range(len(waveform)): # event loop
        ...
        np.append(ped_rms_list, ped_rms)
        ax2.hist(ped_rms, histtype='stepfilled', edgecolor='black',color='dodgerblue',linewidth=2,label='pedestal RMS',alpha=0.6)
        if ped_rms > 0.15:
...
#        if (peak_time) < 0.7e-8 or (peak_time) > 1.2e-8:
        if (peak_time) < 0 or (peak_time) > 2.5e-9:
            timestamp = np.append(timestamp, -99.0)
            continue
        ...
        before = peak_index - 40  # Find the maximum index before the peak index
    ...
    #print(p0)
    popt, _ = curve_fit(landau, ben_center, counts, p0=p0, maxfev=10000) # fitting
...
#    timestamp_x = np.linspace(timestamp_fit-5*timestamp_sigma, timestamp_fit+5*timestamp_sigma, len(bins)*100)
    timestamp_x = np.linspace(0, 150, 1500)

    timestamp_sigma_ps = timestamp_sigma * 1e+3  # convert to ps

    fig1, ax1 = plt.subplots(figsize=(10, 8)) # pedestal 
    ...
    ax1.hist(timestamp, bins=np.arange(0, 250 + bin_width,bin_width),histtype='stepfilled', edgecolor='black',linewidth = 3.0,alpha=0.6, color='b', label='UFSD-LF')
    ax1.plot(timestamp_x, landau(timestamp_x, timestamp_amp, timestamp_fit, timestamp_sigma), 'r-',linewidth = 3.0, label = 'mean = {:.2f} fC \nσ = {:.4f} '.format(timestamp_fit, timestamp_sigma))

    ax1.set_xlim(0, 250)
    ax1.set_ylim(0,150)
    ax1.set_xlabel(r'Charge (fC)')
    ax1.set_ylabel('Counts')
    ax1.legend(frameon=False,loc='best',fontsize=18)
    ax1.grid()
    # fitting sigma
    # save plt pdf

    fig1.savefig(f"{path}/{fname}_collected_charge.png",bbox_inches='tight')
