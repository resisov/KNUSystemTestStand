...
files = glob.glob('output/LF_PreIrrad_Sr90_glich_80uV_with_foil/*.root')
...
#        if (peak_time) < 0.7e-8 or (peak_time) > 1.2e-8:
        ...
        before = peak_index - 120  # Find the maximum index before the peak index
        after = peak_index + 280  # Find the minimum index after the peak index
...
#    timestamp_x = np.linspace(timestamp_fit-5*timestamp_sigma, timestamp_fit+5*timestamp_sigma, len(bins)*100)
    ...
    plt.hist(timestamp, bins=np.arange(0, 250 + bin_width,bin_width),histtype='stepfilled', edgecolor='black',linewidth = 3.0,alpha=0.6, color='b', label='UFSD-LF')
    
    plt.plot(timestamp_x, landau(timestamp_x, timestamp_amp, timestamp_fit, timestamp_sigma), 'r-',linewidth = 3.0, label = 'mean = {:.2f} fC \nσ = {:.4f} '.format(timestamp_fit, timestamp_sigma))
