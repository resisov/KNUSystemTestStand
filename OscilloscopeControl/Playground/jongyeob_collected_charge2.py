...
def gauss_landau(x, A1, A2, mpv1, mpv2, eta1, eta2,C=0):
...
#        if (peak_time) < 0.7e-8 or (peak_time) > 1.2e-8:
        ...
        after = peak_index + 350  # Find the minimum index after the peak index
    ...
    timestamp = timestamp[mask]/gain[j]
    ...
    plt.hist(timestamp, bins=np.arange(0, 10 + bin_width,bin_width),histtype='stepfilled', edgecolor='black',linewidth = 3.0,alpha=0.6, color='b', label='UFSD-LF')

    plt.xlabel(r'$Q_{charge}$ / Gain (fC)')
    plt.ylabel('Counts')
