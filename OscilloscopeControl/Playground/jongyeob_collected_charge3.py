...
def double_fitting(x, A1, A2, mpv1, mpv2, eta1, eta2,C=0):
...
#        if (peak_time) < 0.7e-8 or (peak_time) > 1.2e-8:
        ...
        before = peak_index - 100  # Find the maximum index before the peak index
        after = peak_index + 150  # Find the minimum index after the peak index
    ...
    #print(p0)
    popt, _ = curve_fit(double_fitting, ben_center, counts, p0=p0, maxfev=10000) # fitting
    A1, A2, mpv1, mpv2, eta1, eta2 = popt
    eta1 = abs(eta1) # make sure sigma is positive
...
#    timestamp_x = np.linspace(timestamp_fit-5*timestamp_sigma, timestamp_fit+5*timestamp_sigma, len(bins)*100)
    ...
    plt.plot(x, double_fitting(x,A1,A2,mpv1,mpv2,eta1,eta2), 'r-',linewidth = 3.0,label = 'Gaussian + Landau fit')
    #plt.text(0.5, 0.5, f'Noise mean: {mpv1:.2f} fC, Signal MPV: {mpv2:.2f} fC\nNoise sigma: {eta1:.2f} fC, Signal eta: {eta2:.2f} fC',fontsize=10, transform=plt.gca().transAxes, verticalalignment='top')
    plt.text(0.35, 0.5, f'Gaussian mean: {mpv1:.2f} fC, $\sigma$: {eta1:.2f} fC\nLandau MPV: {mpv2:.2f} fC',fontsize=20, transform=plt.gca().transAxes, verticalalignment='top')
    ...
    sigma = np.append(sigma, mpv2)

print("collected charge:\n", np.array2string(sigma, separator=','))
#print("\nno bin : ", sigma)